...
# import random
from ImageBase import loadImg, writeImg, adjustBrightnessAndContrast, GammaCorrection
...
class ImageColorAug():
    ...
    def __init__(self, file, dstImgPath):
        self.image = loadImg(file, toRgb=False)
    ...
    def _augmentByBrightnessAndContrast(self, numbers):
        beta = 50
        for alpha in np.linspace(0.1, 2, numbers):
        ...
        alpha = 0.6
        for beta in np.linspace(1, 200, numbers):
            newName = r'/' + self.imgName + 'alphaBeta' + '_' + str(alpha) + '_' + str(beta) + '.png'
            print('newName=', newName)
            img = adjustBrightnessAndContrast(self.image.copy(), alpha, beta)
            writeImg(img, self.dstImgPath + newName)
    ...
    def _augmentByGammaCorrection(self, numbers):
        for gamma in np.linspace(0.1, 4, numbers):
            # gamma = gamma.round(2)
